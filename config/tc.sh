...
tc qdisc del dev ${IFTOR} root

#add root class
tc qdisc add dev ${IF} root handle 1: htb default 10
tc qdisc add dev ${IFTOR} root handle 1: htb default 10

#add parent class
tc class add dev ${IF} parent 1: classid 1:1 htb rate ${LINKCEIL} ceil ${LINKCEIL}
tc class add dev ${IFTOR} parent 1: classid 1:1 htb rate ${LINKCEIL} ceil ${LINKCEIL}

#add our two classes. one unlimited, another limited
tc class add dev ${IF} parent 1:1 classid 1:10 htb rate ${LINKCEIL} ceil ${LINKCEIL} prio 0
tc class add dev ${IF} parent 1:1 classid 1:11 htb rate ${LIMIT} ceil ${LIMIT} prio 1

tc class add dev ${IFTOR} parent 1:1 classid 1:10 htb rate ${LINKCEIL} ceil ${LINKCEIL} prio 0
tc class add dev ${IFTOR} parent 1:1 classid 1:11 htb rate ${LIMIT} ceil ${LIMIT} prio 1

#add handles to our classes so packets marked with <x> go into the class with "... handle <x> fw ..."
tc filter add dev ${IF} parent 1: protocol ip prio 1 handle 1 fw classid 1:10
tc filter add dev ${IF} parent 1: protocol ip prio 2 handle 2 fw classid 1:11
tc filter add dev ${IFTOR} parent 1: protocol ip prio 1 handle 1 fw classid 1:10
tc filter add dev ${IFTOR} parent 1: protocol ip prio 2 handle 2 fw classid 1:11
...
iptables -t mangle -A OUTPUT -p tcp -m tcp --dport 8334 ! -d ${LOCALNET_V4} -j MARK --set-mark 0x2
iptables -t mangle -A OUTPUT -p tcp -m tcp --sport 8334 ! -d ${LOCALNET_V4} -j MARK --set-mark 0x2
